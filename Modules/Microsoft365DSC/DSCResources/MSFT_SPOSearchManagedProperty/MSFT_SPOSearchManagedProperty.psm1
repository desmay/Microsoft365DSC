function Get-TargetResource {
        ...
        try {
                ...
                # Get Alias
                $aliasProperties = $Script:RecentMPExtract.SearchConfigurationSettings.SearchSchemaConfigurationSettings.Aliases.dictionary.KeyValueOfstringAliasInfoy6h3NzC8 `
                | Where-Object -FilterScript { $_.Value.ManagedPid -eq $currentManagedPID }

                $propalias = @()
                foreach ($aliasprop in $aliasProperties) {
                        $propalias += $aliasprop.Key.ToString()
                }
                ...
                return @{
                        ...
                        Searchable                  = $property.Value.Searchable
                        FullTextIndex               = $FullTextIndex
                        FullTextContext             = $property.Value.Context
                        Queryable                   = $property.Value.Queryable
                        Retrievable                 = $property.Value.Retrievable
                        AllowMultipleValues         = $property.Value.HasMultipleValues
                        Refinable                   = $fixedRefinable
                        Sortable                    = $fixedSortable
                        Safe                        = $property.Value.SafeForAnonymous
                        Aliases                     = $propalias
                        TokenNormalization          = $property.Value.TokenNormalization
                        CompleteMatching            = $property.Value.CompleteMatching
                        LanguageNeutralTokenization = $property.Value.LanguageNeutralWordBreaker
                        FinerQueryTokenization      = $property.Value.ExpandSegments
